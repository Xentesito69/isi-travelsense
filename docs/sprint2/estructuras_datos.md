...
| `preferencias` | TEXT | Preferencias de viaje |
...
## 3. Contratos JSON de la API REST

### GET `/api/v1/users` → 200
```json
[{ "id": 1, "nombre": "María García", "email": "m.garcia@email.com", "preferencias": "playa, gastronomía" }]
```

### POST `/api/v1/users` → 201
```json
// Request
{ "nombre": "Juan López", "email": "j.lopez@email.com", "preferencias": "montaña" }
// Response
{ "id": 101, "nombre": "Juan López" }
```

### GET `/api/v1/destinations` → 200
```json
[{ "id": 5, "nombre": "Barcelona", "descripcion": "Arquitectura modernista", "clima": "Húmedo y suave", "pais": "España", "region": "Europa" }]
```

### GET `/api/v1/events` → 200
```json
[{ "id": 12, "nombre": "Festival Primavera Sound", "tipo": "Música", "precio": "180€", "destino_id": 5, "destino_nombre": "Barcelona" }]
```

### GET `/api/v1/itineraries` → 200
```json
[{
  "id": 1, "usuario_nombre": "María García", "destino_nombre": "Barcelona",
  "fecha_inicio": "2025-07-01", "fecha_fin": "2025-07-07", "presupuesto": 1200.0,
  "actividades": [{ "id": 1, "nombre": "Visita Sagrada Familia", "coste": 25.0 }]
}]
```

### POST `/api/v1/populate` → 200
```json
...
# Lista plana resultante: (nombre, descripcion, clima, pais, region)
DESTINOS_POOL = [("Madrid", "Capital cultural...", "Variable", "España", "Europa"), ...]
```
